...
SELECT cli.nome, pedidos.id_pedido,pedidos.data_pedido
FROM clientes AS cli
INNER JOIN pedidos ON cli.id_cliente = 3;

-- SELECIONAR PRODUTOS CUJO PRECO SEJA MAIOR QUE 500
SELECT * FROM produtos WHERE preco > 500; 
-- ou
SELECT nome_produto, preco, estoque FROM produtos WHERE preco > 500;

-- SABER A CIDADE DO CLIENTE
SELECT cli.nome, cidades.nome_cidade
FROM clientes AS cli
INNER JOIN cidades ON cli.id_cidade = cidades.id_cidade;
-- OU
SELECT clientes.nome, cid.nome_cidade
FROM cidades AS cid
INNER JOIN clientes ON clientes.id_cidade = cid.id_cidade;

-- PRECISA IDENTIFICAR O PRODUTO DO PEDIDO, O FUNCIONARIO QUE ATENDEU,
-- E O CLIENTE QUE NÃO FICOU SATISFEITO
SELECT pedidos.data_pedido, funcionarios.nome_funcionario, 
prod.nome_produto, clientes.nome AS nome_cliente
FROM produtos AS prod 
INNER JOIN pedidos ON pedidos.id_produto = prod.id_produto
